...
def calculate_demographic_data(print_data=True):
    ...
    highest_earning_country = (df[df['salary'].str.contains('>50K')].groupby(['native-country']).size() / df.groupby(['native-country']).size()).idxmax()
    highest_earning_country_percentage = round((df[df['salary'].str.contains('>50K')].groupby(['native-country']).size() / df.groupby(['native-country']).size()).max() * 100, 1)

    # Identify the most popular occupation for those who earn >50K in India.
    top_IN_occupation = df.loc[df['native-country'].str.contains('India') & df['salary'].str.contains('>50K'), ["occupation"]].groupby(['occupation']).size().idxmax()
